# Demultiplexing data (ALK) and integration with public data (GSE147821) from Kameneva et al. 2021
...
seurList <- list()
scDIRS <- c("CRSC0","CRSC1","CRSC2","CRSC3","CRSC4")
scnames <- c("sc0","sc1","sc2","sc3","sc4")
...
seurList[[i]] <- CellCycleScoring(seurList[[i]], s.features = s.genes, g2m.features = g2m.genes, set.ident = FALSE)
...
for (i in 1:13){
  ...
  # Add CellCycleMarkers
  seurList[[j]] <- CellCycleScoring(seurList[[j]], s.features = s.genes, g2m.features = g2m.genes, set.ident = FALSE)
...
# Add CellCycleMarkers, excluded because already added separately for each dataset
#seur.combined <- CellCycleScoring(seur.combined, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)


# Run the standard workflow for visualization and clustering
seur.combined <- ScaleData(seur.combined, verbose = FALSE)
seur.combined <- RunPCA(seur.combined, npcs = 50, verbose = FALSE)
...
seur.combined <- FindClusters(seur.combined, resolution = 0.1)
seur.combined <- RunUMAP(seur.combined, reduction = "pca", dims = 1:30)

saveRDS(seur.combined,file="stef_ALK_and_noALK_integrated.rds")

seur.markers <- FindAllMarkers(seur.combined, only.pos = TRUE, min.pct = 0.5, logfc.threshold = 0.5)
write.table(seur.markers,"Ada_plus_stef_ALK_markers_dim30.txt",sep="\t",quote=F)

pdf("DimPlot_ada_plus_ALK_Stef_dim20.pdf")
